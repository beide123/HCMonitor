...
{
  //Creat a empty queue
  KeyQue = malloc( sizeof( struct Queue ) ); 
  QueBak = malloc( sizeof( struct Queue ) );  
  KeyQue->base = (QType*)malloc(BUFSIZE*sizeof(struct node_data));  
  QueBak->base = (QType*)malloc(BUFSIZE*sizeof(struct node_data));  
  if(!KeyQue->base || !QueBak->base)
	rte_exit(EXIT_FAILURE,"OVERFLOW\n");    
  KeyQue->front = KeyQue->rear = 0;
  QueBak->front = QueBak->rear = 0; 
  RxQue = (QType*)malloc(BUFSIZE * sizeof(struct node_data));
  return 1;
}
